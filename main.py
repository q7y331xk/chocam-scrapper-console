from config import EXCEL_KEYWORDS_PATH, OPTION_NEW_TABLE, RDS_CALCULATED_TABLE, RDS_PROCESSED_TABLE, RDS_RAW_TABLE, SERVICE_FEE, SILENCE
...
from service.send_msg import send_msg

# change id and pw

def write(raw_table, processed_table, calculated_table, option_new_table):
    print("========================================")
    print("check crawling options")
    if option_new_table:
        print('crawl into new table')
        create_tables_if_exists_drop(raw_table, processed_table, calculated_table)
    else:
...
write(RDS_RAW_TABLE, RDS_PROCESSED_TABLE, RDS_CALCULATED_TABLE, OPTION_NEW_TABLE)